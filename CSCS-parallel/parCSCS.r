parCSCS<- function( Y, lambda, L=NULL, maxitr=100, tol=1e-4, warmstart=FALSE, clusters=1) {
...
####### Y = X
  ...
  ## eps_log: (p-1)-vector of number maximum difference for considering convergence
    ...
    L[1, 1] = 1/sqrt(S[1,1])
    out = foreach(k=2:p) %dopar% { ## Loop in Algorithm 2
    ...
    list(L=out, itr=itr_log, eps=eps_log)
